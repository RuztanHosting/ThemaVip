...
installTheme(){
    ...
    tar -cvf ThemaVipbackup.tar.gz pterodactyl > /dev/null 2>&1
    echo -e "${GREEN}Installing theme... ${RESET}"
    cd /var/www/pterodactyl > /dev/null 2>&1
    echo -e "${GREEN}Removing old theme if exist${RESET}"
    rm -r ThemaVip > /dev/null 2>&1
    echo -e "${GREEN}Download the Theme${RESET}"
    git clone https://github.com/RuztanHosting/ThemaVip.git > /dev/null 2>&1
    cd ThemaVip > /dev/null 2>&1
    echo -e "${GREEN}Removing old theme resources if exist${RESET}"
    rm /var/www/pterodactyl/resources/scripts/ThemaVip.css > /dev/null 2>&1
    ...
    mv ThemaVip.css /var/www/pterodactyl/resources/scripts/ThemaVip.css > /dev/null 2>&1
    cd /var/www/pterodactyl > /dev/null 2>&1
...
repair(){
    bash <(curl https://raw.githubusercontent.com/RuztanHosting/ThemaVip/main/repair.sh)
}

restoreBackUp(){
    echo "Restoring backup..."
    cd /var/www/ > /dev/null 2>&1
    tar -xvf ThemaVipbackup.tar.gz > /dev/null 2>&1
    rm ThemaVipbackup.tar.gz > /dev/null 2>&1
